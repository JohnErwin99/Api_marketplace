    ...
    if (process.env.D365_URL && process.env.D365_CLIENT_ID) {
      try {
        ...
        const r = await api('GET',
          ...
          `&$filter=emailaddress1 eq '${esc(email)}'&$top=1`);
        if (r.ok) {
          const a = r.json.value && r.json.value[0];
          if (!a || a.cr57d_apimarketplaceapproved !== true)
            return res.json({ email, approved: false, products: [] });
          const list = String(a.cr57d_apimarketplaceaccess || a.cr57d_requestedscopes || '')
