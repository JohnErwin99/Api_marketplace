...
// ---------------------------------------------------------------------------
// Manual-approval watcher — covers approvals done directly in Dynamics
// (someone fills iristel_apimarketplaceaccess on the Account) without any
// Power Automate flow. Polls Dataverse every few minutes; when an account's
// access value appears or changes, sends the granted email to emailaddress1.
// First run baselines silently (no email storm on boot/redeploy).
// ---------------------------------------------------------------------------
const SEEN_PATH = path.join(DATA_DIR, 'approval-emails.json');
const APPROVAL_POLL_MS = Number(process.env.APPROVAL_POLL_MS || 5 * 60 * 1000);

async function pollManualApprovals() {
  const { api } = require('../scripts/d365');
  const r = await api('GET',
    "/accounts?$select=accountid,name,emailaddress1,iristel_apimarketplaceaccess" +
    "&$filter=iristel_apimarketplaceaccess ne null");
  if (!r.ok || !r.json || !Array.isArray(r.json.value)) throw new Error('accounts query failed: ' + r.status);

  let seen; let first = false;
  try { seen = JSON.parse(fs.readFileSync(SEEN_PATH, 'utf8')); }
  catch { seen = {}; first = true; }

  for (const a of r.json.value) {
    const access = String(a.iristel_apimarketplaceaccess || '').trim();
    const email = String(a.emailaddress1 || '').trim().toLowerCase();
    if (!access || !email) continue;
    const prev = seen[a.accountid];
    if (!first && prev !== access) {
      const msg = `Your access for ${access.split(',').join(', ')} has been granted. Please log in to the partner portal (https://www.iristelpartnerportal.com) and navigate to /console.`;
      const sent = await sendMail(email, 'Your Iristel API Marketplace access has been granted', msg);
      console.log(`[onboarding] manual approval detected for ${email} (${access}) — email sent: ${sent}`);
    }
    seen[a.accountid] = access;
  }
  fs.mkdirSync(DATA_DIR, { recursive: true });
  fs.writeFileSync(SEEN_PATH, JSON.stringify(seen, null, 2));
  if (first) console.log(`[onboarding] approval watcher baselined ${Object.keys(seen).length} account(s) — no emails on first run`);
}

function startApprovalWatcher() {
  if (!process.env.D365_URL || !process.env.D365_CLIENT_ID) {
    console.log('[onboarding] approval watcher off — no D365 config');
    return;
  }
  const tick = () => pollManualApprovals().catch((e) => console.warn('[onboarding] approval watcher:', e.message));
  tick();
  setInterval(tick, APPROVAL_POLL_MS).unref();
  console.log(`[onboarding] approval watcher polling Dynamics every ${Math.round(APPROVAL_POLL_MS / 1000)}s`);
}
